classdef DebugGeneratorManyPatch < handle
  ...
  methods
    ...
    function initialize(obj, patch_info, program)
      % Change saccade_struct.total_time to saccade_struct.average_velocity in deg/s
      % In the update function, convert your deg/s value to a px/s value
      %   using pct.util.deg2px_from_config()
      % Also in the update function, use your px/s velocity to calculate
      %   the total time, which you can then use as now to establish a
      %   momentary x, y position
