function patch_3_analysis_selected_parameters()
% To be run in cluster

data_folder = [pct.util.get_project_folder, ...
  '/+pct/+simulations/+data/', ...
  '3-patches-with-val-update-2020-05-13_13-16-06/'];

coop_reward = 1;
m1_error_rate = 0.1;
m2_error_rate = 0.1;

file_identifier = ['patches-3-coop-', num2str(100*coop_reward) , ...
...
fname = [pct.util.get_project_folder, ...
    ...
    '3-patches/3-patch_reward-diff-matrix', ...
...
for moves_ind = 1:3
  ...
  suptitle(['3 patches: Max moves = ', num2str(max_moves(moves_ind))]);
  fname = [pct.util.get_project_folder, ...
    '/+pct/+simulations/+analysis/', ...
    '3-patches/3-patch_reward-diff_max-moves', ...
