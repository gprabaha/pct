function patch_4_analysis_selected_parameters()
% To be run in cluster

data_folder = [pct.util.get_project_folder, ...
  '/+pct/+simulations/+data/', ...
  '4-patches-with-val-update-2020-05-13_21-07-21/'];

coop_reward = 1;
m1_error_rate = 0.1;
m2_error_rate = 0.1;

file_identifier = ['patches-4-coop-', num2str(100*coop_reward) , ...
...
fname = [pct.util.get_project_folder, ...
    ...
    '4-patches/4-patch_reward-diff-matrix', ...
...
for moves_ind = 1:3
  ...
  suptitle(['4 patches: Max moves = ', num2str(max_moves(moves_ind))]);
  fname = [pct.util.get_project_folder, ...
    '/+pct/+simulations/+analysis/', ...
    '4-patches/4-patch_reward-diff_max-moves', ...
